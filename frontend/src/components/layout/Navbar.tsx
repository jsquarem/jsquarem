...
export default function Navbar({ isDark, toggleTheme }: NavbarProps) {
  ...
  const sectionIds = navLinks.filter((l) => !("external" in l && l.external)).map((l) => l.href.replace("#", ""));
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => {
              ...
              const isActive = !isExternal && active === id;
              if (isExternal) {
                return (
                  <a
                    key={link.href}
                    ...
                    className="px-4 py-2.5 rounded-lg text-base font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors duration-200"
                  >
                    {link.label}
                  </a>
                );
